...
import { isArrayPattern, isCallExpression, isIdentifier, isImport, isImportDeclaration, isImportDefaultSpecifier, isImportNamespaceSpecifier, isImportSpecifier, isObjectPattern, isObjectProperty, isStringLiteral, isVariableDeclaration, } from "@babel/types";
...
const ASTParser = {
    ...
    getChildNodes(imports, astToken, props, parent, children) {
        ...
        if (currentNode) {
            ...
            const filePath = ParserHelpers.validateFilePath(path.resolve(path.dirname(parent.filePath), moduleIdentifier));
    ...
    getJSXChildren(astTokens, imports, parentNode) {
        let childNodes = {};
        let props = {};
        let token;
    ...
    getJSXProps(astTokens, startLoc) {
        let j = startLoc;
        const props = {};
    ...
    checkForRedux(astTokens, imports) {
...
const ImportParser = {
     ...
     */
    parseImportDeclaration(declaration) {
        const output = {};
        let importName = "";
        let importAlias;
     ...
     */
    parseVariableDeclaration(declaration) {
        const output = {};
        let importName = "";
        let importAlias;
        /*
                ...
                if (isIdentifier(LHS)) {
                    ...
                    LHS.properties.forEach((objectProperty) => {
                        ...
                        if (isObjectProperty(objectProperty)) {
                            ...
                            importAlias = isIdentifier(alias) ? alias.name : isStringLiteral(alias) ? alias.value : "";
